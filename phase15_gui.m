...
stopRequestedG=false;
previousAnalysisType='Plane Stress';
...
% -------------------------------------------------------------------------
% Refinement region storage (PLACEHOLDER ONLY - not applied to mesh generation)
...
meshRefinementImplemented=false; % placeholder only: regions are visual/selection records and do not alter the mesh
refPickActive=false;
refPickCount =0;
refSelectedType='';
refSelectedID=[];
...
analysisTypeDropDown=uidropdown(engPanel,...
    'Items',{'Plane Stress','Plane Strain','Axisymmetric'},...
...
uilabel(geomPanel,'Text',char(9432),'Position',[224 57 20 22],'FontColor',[0.2 0.5 0.9],...
    'Tooltip','Placeholder only. You can select/store regions, but they are not used by mesh generation.');

uitextarea(geomPanel,'Position',[20 8 200 35],'Editable','off','FontSize',9,...
    'Value',{'Placeholder only: define refinement regions visually.','These regions do not change the generated mesh.'});

% =========================================================================
% Mesh refinement full-left-panel mode
% =========================================================================
meshRefPanel=uipanel(leftPanel,'Title','Mesh Refinement Regions (Placeholder)',...
    'Position',[10 10 255 610],'Visible','off');

uilabel(meshRefPanel,'Text','Placeholder only: pick/store refinement regions for display. Mesh refinement is not implemented yet.',...
...
uilabel(meshRefPanel,'Text','Click the Mesh / Status view to select the active entity type. These selections are visual only.',...
...
refRadiusField=uieditfield(meshRefPanel,'numeric','Position',[95 358 95 22],'Value',0.01);
...
uilabel(meshRefPanel,'Text',char(9432),'Position',[224 358 20 22],'FontColor',[0.2 0.5 0.9],...
    'Tooltip','Placeholder only. For Vertex/Point: circle radius. For Edge: visual band thickness.');

uilabel(meshRefPanel,'Text','Local h','Position',[12 326 70 20]);
refHField=uieditfield(meshRefPanel,'numeric','Position',[95 326 95 22],'Value',0.002);
...
uilabel(meshRefPanel,'Text',char(9432),'Position',[224 326 20 22],'FontColor',[0.2 0.5 0.9],...
    'Tooltip','Placeholder only. Stored local h target; not used by mesh generation.');

uibutton(meshRefPanel,'push','Text','Add Placeholder Region',...
    'Position',[12 286 220 30],'BackgroundColor',[0.1 0.6 0.2],'FontColor','white',...
    'ButtonPushedFcn',@addSelectedRefinementRegionCallback);

uilabel(meshRefPanel,'Text','Placeholder Regions','Position',[12 252 150 20],'FontWeight','bold');
refRegionList=uilistbox(meshRefPanel,'Position',[12 115 220 135],...
    'Items',{'No placeholder regions defined.'},'ItemsData',0,'Value',0);
...
uibutton(meshRefPanel,'push','Text','Exit Placeholder Regions',...
...
uilabel(meshTab,'Text','Refinement Regions (placeholder):',...
...
refSummaryBox=uitextarea(meshTab,'Position',[20 130 735 26],...
    'Editable','off','FontSize',8,'Value',{'No placeholder refinement regions defined.'});
...
postColormapMode='Default'; % dropdown value string
...
grid(bcLoadAx,'on'); axis(bcLoadAx,'equal'); set(bcLoadAx,'TickLabelInterpreter','latex');
bcLoadAx.DataAspectRatio=[1 1 1];
bcLoadAx.PlotBoxAspectRatioMode='auto';
quietAxesToolbar(bcLoadAx);

% Post-processing tab — left control panel
% Layout (y increases upward from bottom of tab, tab inner height ~570px):
%   570 ← Results label + Clear button
%   548 ← listbox top
%   358 ← listbox bottom / hint
%   340 ← View Mode label
%   314 ← Tiled / Sub-Tabs buttons
%   272 ← ▶ Plot button
%   242 ← divider
%   224 ← Display Units label
%   204 ← Color Scheme label
%   180 ← colormap dropdown
%   150 ← Original Mesh Overlay label
%   124 ← On / Off buttons
%    94 ← Value Probe label
%    68 ← On / Off buttons
%    48 ← probe hint label

uilabel(postTab,'Text','Results','Position',[10 548 60 22],'FontWeight','bold');
uilabel(postTab,'Text',char(9432),'Position',[64 548 20 22],...
...
postResultList=uilistbox(postTab,...
    ...
    'Multiselect','on','Value',postSelectedResults,'Position',[10 362 175 182],...
    'ValueChangedFcn',@postResultListChangedCallback);
uilabel(postTab,'Text','click = toggle, ctrl/shift also works',...
    'Position',[10 345 175 15],'FontSize',8,'FontColor',[0.55 0.55 0.55]);

uilabel(postTab,'Text','View Mode','Position',[10 322 175 20],'FontWeight','bold');
postModeTiledBtn=uibutton(postTab,'push','Text','Tiled',...
    'Position',[10 296 82 24],'BackgroundColor',[0.3 0.6 1],'FontColor','white',...
    'ButtonPushedFcn',@(~,~) setPostMode('tiled'));
postModeTabBtn=uibutton(postTab,'push','Text','Sub-Tabs',...
    'Position',[98 296 77 24],'BackgroundColor',[0.9 0.9 0.9],'FontColor','black',...
    'ButtonPushedFcn',@(~,~) setPostMode('subtab'));

uibutton(postTab,'push','Text','▶  Plot',...
    'Position',[10 256 175 32],'BackgroundColor',[0.15 0.45 0.85],'FontColor','white',...
    'FontSize',13,'FontWeight','bold','ButtonPushedFcn',@plotResultCallback);

postUnitsLabel=uilabel(postTab,'Text',sprintf('Units: %s',getDisplayUnit()),...
    'Position',[10 230 175 18],'FontSize',9,'FontColor',[0.3 0.3 0.3]);

uilabel(postTab,'Text','Color Scheme','Position',[10 208 175 20],'FontWeight','bold');
postCmapDropDown=uidropdown(postTab,...
    'Items',{'Default','Berry Cute','The Bon''s Fruit Loops','Party','Popsicles'},...
    'Value','Default',...
    'Position',[10 184 175 22],...
    'Tooltip','Choose a color scheme for contour plots.',...
    'ValueChangedFcn',@(src,~) setPostColormapMode(src.Value));

uilabel(postTab,'Text','Original Mesh Overlay','Position',[10 156 175 20],'FontWeight','bold');
postOverlayOnBtn=uibutton(postTab,'push','Text','On',...
    'Position',[10 130 82 24],'BackgroundColor',[0.3 0.6 1],'FontColor','white',...
...
postOverlayOffBtn=uibutton(postTab,'push','Text','Off',...
    'Position',[98 130 77 24],'BackgroundColor',[0.9 0.9 0.9],'FontColor','black',...
    'Tooltip','Hide the undeformed original mesh overlay.',...
    'ButtonPushedFcn',@(~,~) setPostOverlayMode(false));

uilabel(postTab,'Text','Value Probe','Position',[10 100 175 20],'FontWeight','bold');
postProbeOnBtn=uibutton(postTab,'push','Text','On',...
    'Position',[10 74 82 24],'BackgroundColor',[0.9 0.9 0.9],'FontColor','black',...
...
postProbeOffBtn=uibutton(postTab,'push','Text','Off',...
    'Position',[98 74 77 24],'BackgroundColor',[0.3 0.6 1],'FontColor','white',...
...
postProbeLabel=uilabel(postTab,'Text','Probe off.',...
    'Position',[10 52 175 18],'FontSize',9,'FontColor',[0.35 0.35 0.35]);
...
colormap(postAx,makePostColormap(256));
...
% =========================================================================
    ...
    function analysisTypeChangedCallback(~,~)
        switch analysisTypeDropDown.Value
            case 'Plane Stress'
                previousAnalysisType='Plane Stress';
                engInfo.Value={'Plane Stress: thin parts, out-of-plane stress = 0.'};
                logMessage('Analysis type set to Plane Stress.');
            case 'Plane Strain'
                previousAnalysisType='Plane Strain';
                engInfo.Value={'Plane Strain: long bodies, out-of-plane strain = 0.'};
                logMessage('Analysis type set to Plane Strain.');
            case 'Axisymmetric'
                uialert(fig,[...
                    'Axisymmetric analysis was not implemented in this GUI due to the complexity of the required boundary-condition handling.' newline newline ...
                    'More details about the axisymmetric formulation and future work can be read in the project report.'],...
                    'Axisymmetric Not Implemented');
                logMessage('Axisymmetric selected, but not implemented. Reverted to previous analysis type.');
                analysisTypeDropDown.Value=previousAnalysisType;
                if strcmp(previousAnalysisType,'Plane Strain')
                    engInfo.Value={'Plane Strain: long bodies, out-of-plane strain = 0.'};
                else
                    engInfo.Value={'Plane Stress: thin parts, out-of-plane stress = 0.'};
                end
            otherwise
                analysisTypeDropDown.Value='Plane Stress';
                previousAnalysisType='Plane Stress';
                engInfo.Value={'Plane Stress: thin parts, out-of-plane stress = 0.'};
        end
    end
    ...
    function beginComputation(label)
        ...
        isComputing=true; stopRequested=false; stopRequestedG=false; preComputeModel=model;
    ...
    function finishComputation(msg)
        ...
        isComputing=false; stopRequested=false; stopRequestedG=false; preComputeModel=[];
    ...
    function runStopButtonCallback(~,~)
        if isComputing
            stopRequested=true; stopRequestedG=true; processLabel.Text='Stopping...';
    ...
    function checkStopRequested()
        % MATLAB UI callbacks are cooperative: this drawnow gives the Stop
        % button a chance to run, then the flag is checked immediately.
        drawnow;
        if stopRequested || (~isempty(stopRequestedG) && stopRequestedG)
    ...
    function displayUnitsChangedCallback(~,~)
        ...
        if exist('postUnitsLabel','var') && isvalid(postUnitsLabel)
            postUnitsLabel.Text=sprintf('Units: %s',u);
        end
    ...
    function elementTypeChangedCallback(src,~)
        selectedType=src.Value;
        if strcmp(selectedType,'T3')
            logMessage('Element type set to T3.');
            return;
        end

        if strcmp(selectedType,'Q4')
            uialert(fig, ...
                ['Q4 elements have not been implemented in the working GUI yet.' newline newline ...
                 'A lot of progress was made on the Q4 formulation, but it was not fully integrated into the mesh, solve, and post-processing workflow.' newline newline ...
                 'Please see the project report to learn more about the Q4 work.'], ...
                'Q4 Not Implemented');
            logMessage('Q4 element type selected, but Q4 is not implemented. Reverted to T3.');
        else
            uialert(fig, ...
                [selectedType ' elements have not been implemented in the working GUI.' newline newline ...
                 'For a quick summary of this element type and the planned extension, please see the project report.'], ...
                'Element Type Not Implemented');
            logMessage(sprintf('%s element type selected, but it is not implemented. Reverted to T3.',selectedType));
        end

        % Only T3 is currently connected to the mesh/solve/post-processing workflow.
        src.Value='T3';
    end
    ...
    function warnIfMeshLikelyTooFine(h0_m)
        % The current mesh generator is a blocking MATLAB function. If h0 is
        % extremely small, MATLAB may spend a long time inside Delaunay/mesh
        % loops before the UI can process the Stop button. This guard catches
        % the common accidental case before the heavy work begins.
        if isempty(nodesG) || isempty(h0_m) || h0_m<=0
            return;
        end
        spanX=max(nodesG(:,1))-min(nodesG(:,1));
        spanY=max(nodesG(:,2))-min(nodesG(:,2));
        if spanX<=0 || spanY<=0
            return;
        end
        approxCols=max(1,ceil(spanX/h0_m));
        approxRows=max(1,ceil(spanY/(h0_m*sqrt(3)/2)));
        approxPts=approxCols*approxRows;
        if approxPts>75000
            msg=sprintf(['This element size may create about %.0f candidate mesh points before trimming.\n\n' ...
                'Very small element sizes can make MATLAB appear frozen because the current mesh generator is blocking.\n' ...
                'The Stop button can only respond between solver/mesher checkpoints, not inside a single long Delaunay call.\n\n' ...
                'Continue anyway?'],approxPts);
            choice=uiconfirm(fig,msg,'Very Fine Mesh Warning',...
                'Options',{'Continue','Cancel'},'DefaultOption',2,'CancelOption',2);
            if strcmp(choice,'Cancel')
                stopRequested=true;
                stopRequestedG=true;
                error('USER_STOPPED:ComputationStopped','Mesh generation canceled by user.');
            end
        end
    end
    ...
    function loadMeshCallback(~,~)
        ...
        try
            ...
            % Mesh refinement regions are intentionally NOT used here.
            % They are placeholder/visual records only until real local
            % remeshing support is implemented in a future phase.
            if isfield(model,'refinementRegions') && ~isempty(model.refinementRegions)
                logMessage(sprintf('NOTE: %d placeholder refinement region(s) ignored during mesh generation.',numel(model.refinementRegions)));
            end
    ...
    function enterMeshRefinementRegionsCallback(~,~)
        if isempty(model.mesh)
            ...
            logMessage('Mesh refinement placeholder panel blocked: no mesh exists yet.');
            return;
        end
        uialert(fig,[ ...
            'Mesh refinement has not been implemented yet.' newline newline ...
            'This panel only lets you select and display placeholder refinement regions.' newline ...
            'The selected regions will NOT change the generated mesh, the solve, or post-processing results.' newline newline ...
            'It was left as a placeholder due to time restrictions and would be a larger future addition.'], ...
            'Mesh Refinement Placeholder','Icon','warning');
        showMeshRefinementLeftPanel();
        logMessage('Entered Mesh Refinement Regions placeholder mode. Regions are visual only.');
    end
    ...
    function exitMeshRefinementRegionsCallback(~,~)
        ...
        showStandardLeftPanel();
        redrawMeshAxWithRefinements();
        logMessage('Exited Mesh Refinement Regions placeholder mode.');
    end
    ...
    function refSelectionTypeChangedCallback(~,~)
        ...
        updateRefinementPanelState();
        redrawMeshAxWithRefinements();
        logMessage(['Placeholder refinement selection type set to: ' refSelectionTypeDropDown.Value]);
    end
    ...
    function activateRefPickMode()
        ...
        tabGroup.SelectedTab=meshTab;
        logMessage(sprintf('Placeholder refinement pick active — click the Mesh/Status axes to pick a %s.',refSelectionTypeDropDown.Value));
    end
    ...
    function addSelectedRefinementRegionCallback(~,~)
        ...
        if isempty(refSelectedType)||isempty(refSelectedPoint)
            uialert(fig,'Pick a vertex, edge, node, or point first. This is placeholder selection only.','No Placeholder Target'); return; end
        ...
        if isempty(R_m)||R_m<=0
            if strcmp(refSelectedType,'Edge')
                uialert(fig,'Enter a visual band thickness > 0 before adding an edge placeholder region.','Invalid Thickness');
            else
                uialert(fig,'Enter a visual radius > 0 before adding a placeholder region.','Invalid Radius');
            end
            return;
        end
        if isempty(h_m)||h_m<=0
            uialert(fig,'Enter a local h target > 0 before adding a placeholder region. This value is stored but does not refine the mesh.','Invalid h'); return; end
        ...
        if strcmp(shapeType,'EdgeBand')
            logMessage(sprintf('Placeholder refinement #%d added from Edge %d: visual band thickness=%.4g m  h_target=%.4g m (not applied to mesh)',...
                refPickCount,refSelectedID,R_m,h_m));
        else
            logMessage(sprintf('Placeholder refinement #%d added from %s: center=(%.4g, %.4g) m  R=%.4g m  h_target=%.4g m (not applied to mesh)',...
    ...
    function deleteSelectedRefinementCallback(~,~)
        if isempty(model.refinementRegions)
            uialert(fig,'No placeholder refinement regions to delete.','None'); return; end
        idx=refRegionList.Value;
        if isempty(idx)||idx==0||idx>numel(model.refinementRegions)
            uialert(fig,'Select a placeholder refinement region from the list first.','No Region Selected'); return;
        end
        deletedID=model.refinementRegions(idx).regionID;
        model.refinementRegions(idx)=[];
        logMessage(sprintf('Deleted placeholder refinement region #%d.',deletedID));
    ...
    function clearAllRefinementsCallback(~,~)
        ...
        logMessage('All placeholder refinement regions cleared.');
        redrawMeshAxWithRefinements();
        updateRefSummary(); updateRefinementPanelState();
    end
    ...
    function updateRefinementPanelState()
        ...
        if exist('refRegionList','var')&&isvalid(refRegionList)
            ...
            if n==0
                refRegionList.Items={'No placeholder regions defined.'};
    ...
    function drawEdgeBandRegionOnAx(targetAx,rr)
        if isfield(rr,'edgePolyline')&&~isempty(rr.edgePolyline)
            xEdge=rr.edgePolyline(:,1); yEdge=rr.edgePolyline(:,2);
        else
            [xEdge,yEdge]=getEdgePolyline(rr.sourceID);
        end
        if numel(xEdge)<2, return; end

        % Visual band = actual mesh triangles whose centroids are within the
        % edge-band thickness. This makes the band appear only on the meshed
        % interior side of the boundary, instead of drawing an arbitrary
        % circle or an outside offset line.
        if ~isempty(model.mesh)
            conn=model.mesh.connectivity;
            mx=model.mesh.xcoords(:); my=model.mesh.ycoords(:);
            elemCx=mean(mx(conn),2);
            elemCy=mean(my(conn),2);
            d=distancePointsToPolyline(elemCx,elemCy,xEdge,yEdge);
            elemsInBand=find(d<=rr.radius);
            if ~isempty(elemsInBand)
                for ii=1:numel(elemsInBand)
                    e=elemsInBand(ii);
                    nd=conn(e,:);
                    patch(targetAx,mx(nd),my(nd),[0 0.78 0.78],...
        ...
        % Draw the selected boundary edge on top of the shaded band.
        plot(targetAx,xEdge,yEdge,'-','Color',[0 0.78 0.78],'LineWidth',4,...
            'HitTest','off','PickableParts','none');
        plot(targetAx,xEdge,yEdge,'--','Color',[0 0.5 0.5],'LineWidth',1.2,...
            'HitTest','off','PickableParts','none');

        labelPt=rr.center;
        text(targetAx,labelPt(1),labelPt(2),...
            sprintf('R%d EdgeBand  thk=%.3g  h=%.3g',rr.regionID,rr.radius,rr.h_target),...
            'FontSize',7,'Color',[0 0.5 0.5],...
    ...
    function updateRefSummary()
        n=numel(model.refinementRegions);
        if n==0, refSummaryBox.Value={'No placeholder refinement regions defined.'}; return; end
        parts=cell(1,n);
        for k=1:n
            rr=model.refinementRegions(k);
            if isfield(rr,'sourceType')&&~isempty(rr.sourceType), src=rr.sourceType; else, src='Point'; end
    ...
    function solveCallback(~,~)
        ...
        try
            ...
            if strcmp(analysisType,'Axisymmetric')
                error('Axisymmetric analysis is not implemented in this GUI. See the project report for details.');
            end
            E           = eField.Value;
            nu          = nuField.Value;
    ...
    function setPostColormapMode(mode)
        postColormapMode = mode;
        logMessage(sprintf('Color scheme: %s.',mode));
        if postHasResults
            plotResultCallback([],[]);
        end
    end
    ...
    function plotOneResult(tAx,resultType)
        ...
        cb = colorbar(tAx);
        colormap(tAx,makePostColormap(256));
    ...
    function cmap = makePostColormap(n)
        % Named color schemes (postColormapMode is the dropdown string):
        %   'Default'               : MATLAB jet-style (dark blue->cyan->yellow->red->dark red)
        %   'Berry Cute'            : blue (#67A6D4) -> pink (#E1628B)  [original 2-stop]
        %   'The Bon''s Fruit Loops': purple->blue->green->yellow->red  [pastel 5-stop]
        %   'Party'                 : neon blue->pink->orange->green->cyan
        %   'Popsicles'             : blue->white->red  [diverging]
        if nargin < 1 || isempty(n), n = 256; end
        t = linspace(0,1,n)';

        switch postColormapMode
            case 'Berry Cute'
                lo = [103 166 212]/255;   % #67A6D4 blue
                hi = [225  98 139]/255;   % #E1628B pink
                cmap = (1-t).*lo + t.*hi;

            case 'The Bon''s Fruit Loops'
                stops = [160  92 212;     % #A05CD4 violet  (min)
                         103 166 212;     % #67A6D4 bleu
                         130 186 109;     % #82BA6D vert
                         225 165  87;     % #E1A557 jaune
                         225  98 139]/255;% #E1628B rouge   (max)
                cmap = interpStops(stops,t);

            case 'Party'
                stops = [  0  80 255;     % neon blue   (min)
                         255  20 147;     % neon pink
                         255 140   0;     % neon orange
                          57 255  20;     % neon green
                           0 255 255]/255;% neon cyan   (max)
                cmap = interpStops(stops,t);

            case 'Popsicles'
                stops = [  0  48 160;     % deep blue   (min)
                         103 166 212;     % soft blue
                         255 255 255;     % white        (mid)
                         225 130 130;     % soft red
                         192   0   0]/255;% deep red     (max)
                cmap = interpStops(stops,t);

            otherwise  % 'Default' — MATLAB jet-style
                stops = [  0   0 143;     % dark blue   (min)
                           0   0 255;     % blue
                           0 255 255;     % cyan
                         255 255   0;     % yellow
                         255   0   0;     % red
                         128   0   0]/255;% dark red     (max)
                cmap = interpStops(stops,t);
        end
    end

    function cmap = interpStops(stops,t)
        nS = size(stops,1);
        tS = linspace(0,1,nS);
        cmap = zeros(numel(t),3);
        for ch = 1:3
            cmap(:,ch) = interp1(tS, stops(:,ch), t, 'linear');
        end
    ...
    function lims=computeBCAxesLims()
        % Boundary-condition picking should preserve the actual geometry shape.
        % Earlier versions used axis normal, which filled the panel but could
        % visually stretch/squish the part after loading geometry. This computes
        % limits that match the axes box aspect ratio while keeping 1:1 data
        % scaling, so the geometry is not distorted and the view still fills the
        % available UI space as much as possible.
        lims=[];
        if ~isempty(model.mesh)
        ...
        xPad=max(0.06*xr,0.015*max(xr,yr));
        yPad=max(0.12*yr,0.015*max(xr,yr));
        lims=[xmin-xPad, xmax+xPad, ymin-yPad, ymax+yPad];
        lims=matchLimsToAxesAspect(lims,bcLoadAx);
    end

    function lims=matchLimsToAxesAspect(lims,targetAx)
        if isempty(lims)||numel(lims)~=4||~all(isfinite(lims)), return; end
        try
            pos=targetAx.InnerPosition;
            if isempty(pos)||numel(pos)<4||pos(3)<=0||pos(4)<=0
                pos=targetAx.Position;
            end
        catch
            pos=targetAx.Position;
        end
        targetAspect=max(pos(3),1)/max(pos(4),1); % width / height
        xmid=mean(lims(1:2)); ymid=mean(lims(3:4));
        xr=max(diff(lims(1:2)),eps); yr=max(diff(lims(3:4)),eps);
        currentAspect=xr/yr;
        if currentAspect>targetAspect
            yrNew=xr/targetAspect;
            lims(3:4)=ymid+[-0.5 0.5]*yrNew;
        else
            xrNew=yr*targetAspect;
            lims(1:2)=xmid+[-0.5 0.5]*xrNew;
        end
    end
    ...
    function applyBCAxesView()
        ...
        bcLoadAx.DataAspectRatio=[1 1 1];
        bcLoadAx.DataAspectRatioMode='manual';
        bcLoadAx.PlotBoxAspectRatioMode='auto';
