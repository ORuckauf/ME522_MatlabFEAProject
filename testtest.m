...
model.material.analysisType='plane stress';
model.mesh.elementType='Q4';
model.material.E=70e9;
model.material.nu=0.3;
model.material.density=7850;
model.material.thickness=0.01;

% Global system
% model.mesh.xcoords=[0;1;3;0;2;3];
% model.mesh.ycoords=[1.5;1.5;1.5;0;0;0];
% model.mesh.connectivity=[1 4 5 2;2 5 6 3];
model.mesh.xcoords=[0;1;1;0];
model.mesh.ycoords=[0;0;1;1];
model.mesh.connectivity=[1 2 3 4];
...
% model.constraints(1).affected_nodes=[1 4];
% 
% model.constraints(2).type='constraint';
% model.constraints(2).x_mag=NaN;
% model.constraints(2).y_mag=0;
% model.constraints(2).affected_nodes=[4 5 6];
% 
% model.loads(1).type='traction load';
% model.loads(1).x_mag=(1000+1000)/1.5/model.material.thickness;
% model.loads(1).y_mag=0;
% model.loads(1).affected_nodes=[3 6];
...
model.constraints(2).x_mag=NaN;
model.constraints(2).y_mag=0;
model.constraints(2).affected_nodes=[1 2];

model.loads(1).type='traction load';
model.loads(1).x_mag=1000;
model.loads(1).y_mag=0;
model.loads(1).affected_nodes=[2 3];

% model.loads(count).type='point load';
% model.loads(count).x_mag=1000;
% model.loads(count).y_mag=0;
% model.loads(count).affected_nodes=3;
% count=count+1;
%
% model.loads(count).type='point load';
% model.loads(count).x_mag=1000;
% model.loads(count).y_mag=0;
% model.loads(count).affected_nodes=6;
% count=count+1;
...
for II=1:length(model.loads)
    ...
    elseif strcmp(model.loads(II).type,'traction load')
        ...
        temp_conn.nodes=zeros(nElem,nNodes_elem);
        for JJ=1:nElem
        ...
        for KK=1:length(temp_conn.elem)
            % Extract general interpolation matrix and modify according to
            % which nodes are on the edge
            G_tract=model.mesh.GMat{temp_conn.elem(KK)};
            for LL=1:nNodes_elem
                temp_dofs=func_dofs(LL);
                G_tract(:,temp_dofs)=G_tract(:,temp_dofs)*temp_conn.nodes(LL)
            end

            % The traction vector is based on the input magnitudes
            w_tract=[model.loads(II).x_mag; ... % x
                model.loads(II).y_mag];         % y

            % The Jacobian is related to the length of the edge
            % Replace the logical indexing with global node numbers
            temp_conn.nodes(KK,:)=model.mesh.connectivity(temp_conn.elem(KK,1),:).*temp_conn.nodes
            temp_conn.nodes_trunc=temp_conn.nodes(KK,~(temp_conn.nodes(KK,1:min(nNodes_elem,4))==0)); % relevant global node numbers
            temp_conn.coords=[model.mesh.xcoords(temp_conn.nodes_trunc), ...
                model.mesh.ycoords(temp_conn.nodes_trunc)]
            L=sqrt((temp_conn.coords(1,1)-temp_conn.coords(2,1))^2+ ...
                (temp_conn.coords(1,2)-temp_conn.coords(2,2))^2)
            J_tract=L/2

            % Integration using Gaussian quadrature
            np=1; % still need to refine this
            ...
            [s0,t0]=func_edge_map(model.mesh.elementType,find(temp_conn.nodes),r);
            ...
            temp=func_dofs(model.mesh.connectivity(temp_conn.elem(KK,1),:));
